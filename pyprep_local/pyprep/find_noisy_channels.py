...
from pyprep_local.pyprep.utilities import filter_design
...
class NoisyChannels:
    ...
    def __init__(self, raw, do_detrend=True):
        ...
        self.ch_names_new = list(np.asarray(ch_names)[eeg_index])
        # print(self.ch_names_new)
    ...
    def find_bad_by_hf_noise(self, HF_zscore_threshold=5.0):
        ...
        if self.sample_rate > 100:
            ...
            zscore_hf_noise = np.divide(
                np.subtract(noisiness, noisiness_median), noiseSD
            )
            hf_noise_channel_mask = [False] * self.new_dimensions[0]
            for i in range(0, self.new_dimensions[0]):
                hf_noise_channel_mask[i] = zscore_hf_noise[
                    i
                ] > HF_zscore_threshold or np.isnan(zscore_hf_noise[i])
            hf_noise_channels = self.channels_interpolate[hf_noise_channel_mask]
        else:
            EEG_filt = data_tmp
            noisiness_median = 0
            noisinessSD = 1
            zscore_hf_noise = np.zeros((self.new_dimensions[0], 1))
            hf_noise_channels = []
        self.EEGData_beforeFilt = data_tmp
        self.EEGData = np.transpose(EEG_filt)
        for i in range(0, len(hf_noise_channels)):
            self.bad_by_hf_noise.append(self.ch_names_original[hf_noise_channels[i]])
        return None
    ...
    def find_bad_by_correlation(
        ...
        """
        self.find_bad_by_hf_noise()  # since filtering is performed there
    ...
    def find_bad_by_ransac(
        ...
        """
        eeg_index = mne.pick_types(self.raw_mne.info, eeg=True, eog=False, meg=False)
        ch_names = self.raw_mne.info["ch_names"]
        self.ch_names_new = np.asarray((np.asarray(ch_names)[eeg_index]))
        self.n_chans_new = len(self.ch_names_new)
