function [x_keep, y_keep, offset, lag, h] = align_events_val(x,y,tol)
% align two timeseries of irregular, discrete events,
% we have event TIMESTAMPS and event VALUES, both are used to find a match.
...
% inputs
% x,y are both timeseries matrices with 2 columns:
% TIMESTAMP and VALUE
...
assert(length(y)>2,'y must have 2 columns: TIMESTAMP and VAL')

%% Match on DIFF between TIMESTAMPS
diffx = diff(x(:,1));
diffy = diff(y(:,1));
