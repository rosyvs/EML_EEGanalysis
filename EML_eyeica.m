...
addpath(getpath('C:\Users\roso8920\Documents\MATLAB\eeglab_current\eeglab2021.0\plugins\eye-eeg0.85')
...
for s = 1:length(sublist)
    ...
    % use Fieldtrip to load fif data from MNE (TODO can EEGlab read
    % directly??)
    cfg = [];
    cfg.dataset = fullfile(dir_pre, [pID '_p.fif']);
    EEG_ft = ft_preprocessing(cfg);
    
    % TODO read info txt to determine whether EEG+triggers are from SD card
    ...
    logtrig.eegSD_est_diff_since_last = [0; diff(logtrig.eegSD_sample_est)];
    eeg_log_jitter = range(logtrig.diff_since_last - logtrig.eegSD_est_diff_since_last)
    
    %% add blink, saccade and fixation events to the data
    ...
    ix = ~isnan(logtrig.eye_sample + logtrig.eegSD_sample_est);
    F = griddedInterpolant(logtrig.eye_sample(ix), logtrig.eegSD_sample_est(ix), 'spline');
    ...
    hold on
    plot(logtrig.eye_sample(ix), logtrig.eegSD_sample_est(ix),'ro')
