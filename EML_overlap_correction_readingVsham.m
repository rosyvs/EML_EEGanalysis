...
for s = 1:length(sublist)
    ...
    %% design
    cfgDesign = [];
    cfgDesign.eventtypes = {'fix_L'};
    % We use intercept-only formulas because we are only interested in the overlap for now
    cfgDesign.formula = {'y ~ 1 + task'};
    EEG = uf_designmat(EEG,cfgDesign);
    
    EEG = uf_timeexpandDesignmat(EEG,'timelimits',[-0.2 1]);
    ...
    EEG_epoch = uf_epoch(EEG,'timelimits',[-0.2 1]);
    ...
    %
    %    uf_erpimage( EEGd ,'type','modelled','addResiduals',1,'channel',1,'split_by','task')
    %
