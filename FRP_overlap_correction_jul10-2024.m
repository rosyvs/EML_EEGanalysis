...
sublist = [27:181]; % TODO: replace with full sublist, short list used for dev
%%%%%%%%%
...
for s = 1:length(sublist)
    try
        ...
        task_events.index = repmat(0, height(task_events),1); 
        ...
        fixations.type = repmat({'fix_R_reparsed'}, height(fixations),1);
        ...
        % old_fixations = old_fixations(strcmp(old_fixations.task, 'reading'),{'identifier','type','latency','duration'});
        % new_fixations = fixations(strcmp(fixations.task, 'reading'),{'identifier','type','latency','duration'});
        ...
        %% make new eye_events using old blinks and new fixations (and old fixations for between reading pages??)
        % just use right eye blinks for now
        blinks = eye_events(~cellfun('isempty',strfind(eye_events.type, 'blink_R')),:);
        blinks.index = repmat(0, height(blinks),1); % TODO if you care about individual blinks, enumerate per event
        fixations.index = fixations.CURRENT_FIX_INDEX;
        % columns to use 
        cols = {'type','latency','duration','task','identifier','index'};
        events_new = [blinks(:,cols); fixations(:,cols); task_events(:,cols)]; 
        ...
        %% design
        cfgDesign = [];
        cfgDesign.eventtypes = {'fix_R_reparsed','blink_R','task_event'}; % use right eye
        cfgDesign.formula = {'y ~ 1 + cat(task)','y ~ 1', 'y ~ 1 + cat(task)'}; % intercept only - extract unfolded FRP but do condition analyses later
